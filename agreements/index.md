---
layout: default
title: Agreements & Forms
description: Standard legal terms and business forms for KOSLAB PLT.
body_class: agreements-page
general_terms_version: 20260218
ps_terms_version: 20260218
training_terms_version: 20260218
whitelabel_terms_version: 20260218
teaming_terms_version: 20260303
ea_version: 20260218
master_teaming_version: 20260218
order_form_version: 20260220
oppty_form_version: 20260220
---

# Forms & Terms Of Service

Standard legal frameworks and business documents to streamline our collaboration and ensure transparency.

## Business Forms

- [Order Form]({{ '/assets/agreements/order-form-' | append: page.order_form_version | append: '.odt' | relative_url }}) ![ODT Icon]({{ '/assets/images/icons/odt.svg' | relative_url }}){: .icon-download }
- [Opportunity Registration Form]({{ '/assets/agreements/opportunity-registration-form-' | append: page.oppty_form_version | append: '.odt' | relative_url }}) ![ODT Icon]({{ '/assets/images/icons/odt.svg' | relative_url }}){: .icon-download }

## Terms of Service

- [General Terms of Engagement]({{ '/assets/agreements/general-terms-of-engagement-' | append: page.general_terms_version | append: '.pdf' | relative_url }}) ![PDF Icon]({{ '/assets/images/icons/pdf.svg' | relative_url }}){: .icon-download }
- [Professional Services Terms of Service]({{ '/assets/agreements/professional-services-tos-' | append: page.ps_terms_version | append: '.pdf' | relative_url }}) ![PDF Icon]({{ '/assets/images/icons/pdf.svg' | relative_url }}){: .icon-download }
- [Training Terms of Service]({{ '/assets/agreements/training-tos-' | append: page.training_terms_version | append: '.pdf' | relative_url }}) ![PDF Icon]({{ '/assets/images/icons/pdf.svg' | relative_url }}){: .icon-download }
- [Professional Services White Labeling Terms of Service]({{ '/assets/agreements/professional-services-white-labeling-agreement-' | append: page.whitelabel_terms_version | append: '.pdf' | relative_url }}) ![PDF Icon]({{ '/assets/images/icons/pdf.svg' | relative_url }}){: .icon-download }

## Pre-Sales Teaming Terms & Conditions

- [General Teaming Terms and Conditions]({{ '/assets/agreements/general-teaming-terms-' | append: page.teaming_terms_version | append: '.pdf' | relative_url }}) ![PDF Icon]({{ '/assets/images/icons/pdf.svg' | relative_url }}){: .icon-download }

## Agreement Templates

- [Enterprise Agreement]({{ '/assets/agreements/enterprise-agreement-' | append: page.ea_version | append: '.odt' | relative_url }}) ![ODT Icon]({{ '/assets/images/icons/odt.svg' | relative_url }}){: .icon-download }
- [Master Teaming Agreement]({{ '/assets/agreements/master-teaming-agreement-' | append: page.master_teaming_version | append: '.odt' | relative_url }}) ![ODT Icon]({{ '/assets/images/icons/odt.svg' | relative_url }}){: .icon-download }
